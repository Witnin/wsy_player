...
class WsyPlayer extends FijkPlayer {
  static const asset_url_suffix = "asset:///";

  static const cache_switch = 'ijkio:cache:ffio:';

  static String _cachePath = '/storage/emulated/0/Android/data/com.wsy.getx.flutter_getx/files';
  ...
  Future<void> setDataSource(String path,
    ...
    if (File(videoPath).existsSync()){
      ...
      path = videoPath;
      print('getx- play cache video: $path');
    }else if(enableCache){
      //走三级缓存，并添加到二级缓存到本地磁盘
      // 设置播放器缓存的步骤：
      // 1、增加视频path前缀
      ...
      print('getx- play http with cache: $cache_switch$path');
    }else{
      print('getx play http$path');
    }
  ...
  String getVideoCachePath(String url,String cachePath){
    ...
    print('getx- get video Path: $path');
    return path;
  }
}
